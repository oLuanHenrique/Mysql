...
app.route('/clientes')
    ...
    // Consulta os clientes no banco de dados
        connection.query('SELECT * FROM clientes', (err, results) => {
            ...
            'Erro ao buscar clientes no banco de dados' });
            } else {
                res.json(results);
            }
        });
    ...
    // Insere um novo cliente no banco de dados
        ...
        connection.query('INSERT INTO clientes (nome, email) VALUES (?, ?)',
            [nome, email], (err) => {
                if (err) {
                    console.error(err);
                    res.status(500).json({ error:
    ...
    // Atualiza um cliente no banco de dados
        ...
        connection.query('UPDATE clientes SET nome = ?, email = ? WHERE id = ?',
            [nome, email, id], (err) => {
                if (err) {
                    console.error(err);
                    res.status(500).json({ error:
    ...
    // Deleta um cliente do banco de dados
        const { id } = req.body;
        connection.query('DELETE FROM clientes WHERE id = ?',
            [id], (err) => {
                if (err) {
                    console.error(err);
                    res.status(500).json({ error:
